...
export function LoginScreen() {
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 p-4 dark:from-slate-950 dark:via-slate-900 dark:to-emerald-950">
      {/* decorative blobs */}
      ...
      <div className="glass relative z-[1] w-full max-w-md rounded-3xl p-8 shadow-2xl shadow-emerald-900/10">
        {/* Logo */}
        <div className="mb-6 flex flex-col items-center text-center">
          <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-xl shadow-emerald-500/30 ring-4 ring-white/40 dark:ring-white/10">
            {settings?.logo ? (
          ...
          <h1 className="font-serif text-3xl font-bold tracking-tight">
            <span className="text-emerald-600 dark:text-emerald-400">Edu</span>
            <span>Center</span>
            <span className="text-emerald-600 dark:text-emerald-400">JM</span>
          </h1>
        ...
        {mode === 'login' && (
          <>
              ...
              {(['Staff', 'Student'] as Portal[]).map((p) => (
                <button
                  ...
                  className={cn(
                    ...
                    portal === p ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400' : 'border-transparent text-muted-foreground hover:text-foreground'
                  )}
                >
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <Button type="submit" disabled={loading} className="w-full bg-emerald-600 text-white shadow-lg shadow-emerald-600/25 transition hover:bg-emerald-700 hover:shadow-emerald-600/40 active:scale-[0.98]">
                {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <LogIn className="h-4 w-4" />}
                Sign In
              </Button>
              <div className="flex flex-col items-center gap-2 pt-2 text-sm sm:flex-row sm:justify-center">
                <button type="button" onClick={() => setMode('register')} className="text-muted-foreground underline-offset-2 transition hover:text-emerald-600 hover:underline">
                  Register New User
                </button>
                <span className="hidden text-border sm:inline">·</span>
                <button type="button" onClick={() => setMode('forgot')} className="text-muted-foreground underline-offset-2 transition hover:text-emerald-600 hover:underline">
                  Forgot Password?
                </button>
              </div>
            </form>

            <div className="mt-6 rounded-xl border border-dashed border-emerald-200 bg-emerald-50/40 p-3 dark:border-emerald-900/40 dark:bg-emerald-950/20">
              <p className="mb-2.5 text-center text-xs font-semibold uppercase tracking-wide text-emerald-700 dark:text-emerald-400">Demo accounts — click to autofill</p>
              <div className="grid grid-cols-3 gap-2">
                {demoAccounts.map((a) => (
                  <button key={a.label} onClick={() => quickLogin(a)} className="flex flex-col items-center gap-0.5 rounded-lg border border-border bg-card px-2 py-2.5 text-xs font-medium transition hover:scale-105 hover:border-emerald-400 hover:bg-emerald-50 hover:shadow-sm dark:hover:bg-emerald-950/30">
        ...
        {mode === 'register' && (
          <form onSubmit={handleRegister} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {(['Student', 'Teacher'] as const).map((r) => (
                  <button
                    ...
                    className={cn('flex items-center justify-center gap-2 rounded-lg border py-2.5 text-sm font-medium transition', form.role === r ? 'border-emerald-500 bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300' : 'border-border hover:bg-muted')}
                  >
            ...
            <button type="button" onClick={() => setMode('login')} className="w-full text-center text-sm text-muted-foreground underline-offset-2 hover:text-emerald-600 hover:underline">
              Back to Login
            </button>
          </form>
        )}
        ...
        {mode === 'forgot' && (
          <form onSubmit={handleForgot} className="space-y-4">
            ...
            <button type="button" onClick={() => setMode('login')} className="flex w-full items-center justify-center gap-1 text-sm text-muted-foreground underline-offset-2 hover:text-emerald-600 hover:underline">
